...
module Fluent
  module Plugin
    class HaproxyParser < Parser
      Plugin.register_parser("haproxy_syslog", self)

      REGEXP = /^(?<time>[^ ]*\s*[^ ]* [^ ]*) (?<ps>\w+)\[(?<pid>\d+)\]: (?<c_ip>[\w\.]+):(?<c_port>\d+) \[(?<hatime>.+)\] (?<f_end>.+) (?<b_end>.+)\/(?<b_server>.+) (?<tq>[+-]\d+|\d+)\/(?<tw>[+-]\d+|\d+)\/(?<tc>[+-]\d+|\d+)\/(?<tr>[+-]\d+|\d+)\/(?<tt>[+-]\d+|\d+) (?<status_code>\d+) (?<size>[+-]\d+|\d+) (?<req_cookie>\S+) (?<res_cookie>\S+) (?<t_state>[\w-]+) (?<actconn>[+-]\d+|\d+)\/(?<feconn>[+-]\d+|\d+)\/(?<beconn>[+-]\d+|\d+)\/(?<srv_conn>[+-]\d+|\d+)\/(?<retries>[+-]\d+|\d+) (?<srv_queue>[+-]\d+|\d+)\/(?<backend_queue>[+-]\d+|\d+) \{?(?<req_headers>[^}]*)\}? ?\{?(?<res_headers>[^}]*)\}? ?"(?<method>\w+) (?<path>.+) /
